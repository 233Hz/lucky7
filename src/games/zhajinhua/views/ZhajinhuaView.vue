      ...
      <div class="flex flex-wrap items-center justify-between gap-2 p-2 sm:p-2.5 rounded-lg bg-[#fffef0] border-3 border-[#1a1a1a] shadow-[2px_2px_0px_0px_#1a1a1a]">
        <div class="flex flex-wrap items-center gap-1.5 sm:gap-2">
          <h1 class="text-sm sm:text-lg font-black text-[#1a1a1a] flex items-center gap-1.5 tracking-tight font-mono">
            <Layers class="w-4 h-4 sm:w-5 sm:h-5 text-[#1a1a1a] flex-shrink-0" />
            <span class="truncate max-w-[140px] sm:max-w-xs">{{ roomStore.currentRoom?.name || '炸金花真人联机桌' }}</span>
          </h1>
          <span
        ...
        <div class="text-[11px] sm:text-xs font-mono font-bold text-[#1a1a1a]/80 flex flex-wrap items-center gap-1.5">
          ...
          <span class="bg-white px-2 py-0.5 rounded border border-[#1a1a1a] shadow-[1px_1px_0px_0px_#1a1a1a]">
            闷牌 1x · 看牌 2x
          </span>
        </div>
      </div>
    ...
    <div v-else class="mb-4 pb-3 border-b-4 border-[#1a1a1a]">
      ...
      <div class="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1">
        <h1 class="text-lg sm:text-2xl font-black text-[#1a1a1a] uppercase flex items-center gap-2">
          <Layers class="w-5 h-5 sm:w-6 sm:h-6 text-[#1a1a1a] flex-shrink-0" />
        ...
        <div class="text-[11px] sm:text-xs font-mono font-bold text-[#1a1a1a]/80 flex items-center gap-2">
          ...
          <span class="bg-[#fffef0] px-2 py-0.5 rounded border border-[#1a1a1a] shadow-[1px_1px_0px_0px_#1a1a1a]">
            闷牌 1x · 看牌 2x
          </span>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center justify-center gap-3 sm:gap-6 md:gap-8 relative z-20 pt-2 min-h-[140px]">
        ...
        >
          <PlayerSeat
            ...
            :isHost="opp.isHost"
            :readyStatus="isInRoom && gameStatus === 'waiting' ? opp.readyStatus : undefined"
          />

          <!-- Kick Player for Host (Only in multiplayer room) -->
          <div
        ...
        <!-- Empty Seat Placeholders (Only in multiplayer room) -->
      ...
      <div class="my-6 flex flex-col items-center justify-center relative z-10">
        ...
        <div v-if="gameStatus === 'waiting'" class="px-6 py-4 rounded-xl bg-[#fffef0] border-4 border-[#1a1a1a] shadow-[4px_4px_0px_0px_rgba(26,26,26,1)] text-center max-w-md">
          ...
          <div class="text-sm font-black text-[#1a1a1a]">
            {{ !isInRoom ? '老赵、大飞、阿美已就座，点击下方直接发牌开战！' : waitingStatusText }}
          </div>
      ...
      <div class="flex flex-col items-center relative z-20 pb-2">
        ...
        <div class="flex flex-col items-center mb-3">
          <div class="flex items-center space-x-3 sm:space-x-4 mb-2">
            ...
            <div v-else class="w-20 h-28 rounded-lg border-2 border-dashed border-[#1a1a1a] bg-[#fffef0] flex items-center justify-center text-[#1a1a1a]/50 text-xs font-mono font-bold text-center px-2">
              {{ isInRoom && gameStatus === 'waiting' ? (roomStore.isCurrentUserReady ? '已准备就绪' : '等待准备') : '等待发牌' }}
            </div>
          </div>
        ...
        <div class="flex items-center space-x-4 mb-4">
          <div class="flex items-center space-x-2.5 px-3.5 py-1.5 rounded-lg bg-[#fffef0] border-3 border-[#1a1a1a] shadow-[3px_3px_0px_0px_rgba(26,26,26,1)] relative">
            ...
            <span
              v-if="isInRoom && gameStatus === 'waiting'"
...
const minBet = computed(() => (isInRoom.value ? roomStore.currentRoom?.min_bet : 50) || 50)
...
const localAiOpponents = ref<InGameOpponent[]>([
  {
...
function startNewRound() {
  ...
  if (authStore.profile && authStore.profile.chips < minBet.value) {
    dialog.warning('筹码不足，请先签到领取筹码！', { title: '筹码不足' })
    return
  }
